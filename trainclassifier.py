...
def makeTriggerModel(input_shape = (64,64,1), classes = 3):

    #ft.set_tf_session_for_keras(memory_fraction=1.0)
...
def makePlateModel(input_shape = (400,400,1), classes = 10):
    ...
    X = Conv2D(filters=3, kernel_size= (5,5), strides = (2,2), name='conv1', padding='valid', kernel_initializer='glorot_uniform')(X)
    #X = BatchNormalization(axis = 3, name = 'bn_conv1')(X)
    X = Activation('relu')(X)
    X = MaxPooling2D(pool_size=(3,3), strides=(1,1))(X)

    X = Conv2D(filters=20, kernel_size=(3,3), strides = (2,2), name='conv2', padding='valid', kernel_initializer='glorot_uniform')(X)
    #X = BatchNormalization(axis = 3, name = 'bn_conv2')(X)
    X = Activation('relu')(X)
    X = MaxPooling2D(pool_size=(5,5), strides=(1,1))(X)

    X = Conv2D(filters=30, kernel_size=(3,3), strides = (2,2), name='conv3', padding='valid', kernel_initializer='glorot_uniform')(X)
    #X = BatchNormalization(axis = 3, name = 'bn_conv3')(X)
    X = Activation('relu')(X)
    X = MaxPooling2D(pool_size=(7,7), strides=(1,1))(X)
...
def testModel(model, num_dev_files, dev_files_list, classes, mini_batch_size, imgdim):
    ...
    for bno in tqdm(range(0,num_mini_batches)):
        ...
        X_test_orig, Y_test_orig = ft.load_minibatch(classes, dev_files_list, mini_batch_size, bno, imgdim)

        X_test = X_test_orig/255
        _, Y_test = ft.convert_to_one_hot(classes, Y_test_orig)
...
def minibatchLoader(classes, train_files_list, num_mini_batches, mini_batch_size, bno, imgdim):
    ...
    while bno < num_mini_batches:
        ...
        X_train_orig, Y_train_orig = ft.load_minibatch(classes, train_files_list, mini_batch_size, bno, imgdim)
        X = X_train_orig/255
        _, Y = ft.convert_to_one_hot(classes, Y_train_orig)
...
def trainOnData(dataPath, saveDest , train_loss, num_epochs, mini_batch_size, modeltype, imgdim=(100,100,1)):

    print("Making data set from path ", dataPath)

    num_train_files, num_dev_files, tmp_keys, train_files_list, dev_files_list, classes = ft.make_dataset(dataPath, 0.2, imgdim)
    ...
    if modeltype  == "plate":
        model = makePlateModel(imgdim, len(classes))
        print('Using plate model')
    elif modeltype  == "letters":
        model = makeModel(imgdim, len(classes))
        print('Using letters model')
    elif modeltype == "trigger":
        model = makeTriggerModel(imgdim, len(classes))
        print('Using trigger model')
    else:
        print("Unknown model type [{}], cannot continue further".format(modeltype))
        return
    ...
    X_test_orig, Y_test_orig = ft.load_minibatch(classes, dev_files_list, num_dev_files, 0, imgdim)
    X_test = X_test_orig/255
    _, Y_test = ft.convert_to_one_hot(classes, Y_test_orig)
    ...
    ft.saveModel(model, saveDest, classes)
...
if __name__ == "__main__":
    ...
    ap.add_argument("-mt", "--model.type", default='plate',
        help="'plate' or 'letters' or 'trigger' (without quote)")

    args = vars(ap.parse_args())

    imgdim = (int(args["img.height"]), int(args["img.width"]), int(args["img.chan"]))
    trainOnData(args["data.path"], args["dest.loc"], args["train.loss"], int(args["train.epoch"]), int(args["minibatch.size"]), args["model.type"],imgdim)
